...
### The master realm's own instance resources

A separate question from the machine identity above, and already answered:
the master realm's *own* sign-in surface — the gateway's confidential client
and its secret, the console's public client, the `fabric-operator` role, each
declared operator's grants, and the realm's own canonical `frontendUrl` —
is provisioned by the platform, not by hand, using the Keycloak bootstrap
administrator it already generates
([`applications/core/keycloak-credentials`](../applications/core/keycloak-credentials/)).
[`applications/core/master-instance`](../applications/core/master-instance/)
converges these on every sync, through OpenTofu and the Keycloak provider;
its own credential is generated the same way, by
[`applications/core/master-instance-credential`](../applications/core/master-instance-credential/).
See ADR 0025 in the application repository for why realm bootstrap is
platform composition rather than an extension of the borrowed-operator
...
| `saas-fabric-gateway-oidc` | [`applications/core/master-instance-credential`](../applications/core/master-instance-credential/) |
...
Both use `refreshInterval: "0"`, which is load-bearing for the same reason in
each case: Keycloak writes the admin account into its own database at first
start, and the master realm's `saas-fabric-gateway` client keeps whatever
secret [`master-instance`](../applications/core/master-instance/) last set on
it — a later refresh of either Secret would rotate the value here while
Keycloak kept the original.
...
| Perses has no authentication | `enable_auth` is off. This is an accepted interim state rather than an oversight, and it is bounded by an enforced constraint rather than a note: `exposure.plane: operator` in the service contract, and `check_operator_only_services` refusing any product-plane route to it. Signing in through Keycloak needs a Keycloak client of its own, declared and converged the way [`applications/core/master-instance`](../applications/core/master-instance/) now does for the master realm's instance resources — the mechanism exists, extending it to a Perses client does not yet | an OIDC provider in [`applications/catalogue/perses`](../applications/catalogue/perses/), once a Keycloak client can be created from Git |
