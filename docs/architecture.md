...
| Fabric renders no Perses views yet | The Operations experience — an observability module, its Perses adapter, and the permission checks that gate it — is application source and does not exist. The platform side of the contract is complete: a separately deployable Perses at a stable in-cluster address, which is deliberately an API boundary rather than something embedded in Fabric's own deployment | the application repository; this repository's half is the endpoint declared in [`applications/catalogue/perses`](../applications/catalogue/perses/) |
| Perses client projects are intended, not built | The platform-management use is real today; client projects are a candidate. The blocker is upstream of Perses: with no per-client attribute on telemetry there is nothing for a client's datasource to filter by, so no client capability may be declared yet | [`applications/catalogue/perses`](../applications/catalogue/perses/) |
| Perses has no authentication | `enable_auth` is off and the instance is read-only, so the tailnet is the whole boundary. That is coherent while every viewer is a platform operator and there is nothing client-scoped to see; it stops being sufficient the moment either changes. Signing in through Keycloak needs a declarative path into Keycloak's own configuration, which is the same thing blocking Fabric's service identity above | an OIDC provider in [`applications/catalogue/perses`](../applications/catalogue/perses/), once a Keycloak client can be created from Git |
