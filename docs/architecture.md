...
```text
generated in-cluster          keycloak-admin, grafana-admin
  nothing to choose,          an External Secrets Password generator
  nothing to transport        makes them at first sync
...
`keycloak-admin` and `grafana-admin` used to be on the list above, and are not
any more. They are not credentials anyone needs to *choose*, so nobody does: an
External Secrets `Password` generator creates them at first sync.
